  ...
  render() {
    return (
      <div>
        ...
        <Button color="green">
          <Link to={`/profile/edit/${this.state.user.id}`}>Editovat</Link>
        </Button>
        <Button color="green">
          <Link to="/profile/password">Změnit heslo</Link>
        </Button>
        ...
        <Modal size="small" open={this.state.open} onClose={this.handleDeleteCancel}>
          <Modal.Header>Smazat uživatele {`${this.state.user.firstName} ${this.state.user.lastName}`} ?</Modal.Header>
          <Modal.Content>
          ...
          <Modal.Actions>
            ...
            <Button
              positive
              onClick={() => this.handleDeleteConfirm(this.state.user.id)}
              icon="checkmark"
              labelPosition="right"
              content="Ano"
            />
